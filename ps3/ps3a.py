...
def get_word_score(word, n):
    ...
    returns: int >= 0
    """
    score = sum([SCRABBLE_LETTER_VALUES[ch] for ch in word]) * len(word)
    if len(word) == n:
        score += 50 # bonus
    return score
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while not done:
        print "Current Hand:",
        display_hand(hand_)
        in_word = raw_input('Enter word, or a "." to indicate that you are finished:')
        if in_word == '.':
            done = True
